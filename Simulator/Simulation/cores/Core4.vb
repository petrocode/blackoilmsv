...
Public Class Core
    ...
    Sub Run(ByVal Data As Simulation, ByRef History As Simulation_History)
        ...
        Dim FI_Matrix As Fully_Implicit_Matrix
        Dim Last_FI_Matrix As Fully_Implicit_Matrix
        Dim FullyImplicit_MSE As Iteration_MSE

        Dim IMPES_Matrix As IMPES_Matrix
        Dim IMPES_MSE As Iteration_MSE

        Dim SEQ_Matrix As SEQ_Matrix
        Dim SEQ_MSE As Iteration_MSE


        ' Preparing Simulator   
        History.Clear()
        History.Length = Data.Final_Step

        Set_System_of_Units(Data)
        Prepare_Adjacents(Data)
        Initialize_Grids_Porosities(Data)
        Calculate_Grids_Volume(Data)
        Initialize_Gas_Critical_Pressure_Temperature(Data)
        Prepare_Implicit_Matrix(Data, FI_Matrix)
        Prepare_Implicit_Matrix(Data, Last_FI_Matrix)
        Prepare_IMPES_Matrix(Data, IMPES_Matrix)
        'Prepare_SEQ_Matrix(Data, IMPES_Matrix)
        History.Cast(Data)
        ...
        Do While (Data.Total_Time < Data.Finish_Time And Data.Current_Step < Data.Final_Step) And Not Paused
            ...
            Data.Current_Step += 1
            History.Current = Data.Current_Step
            FI_Matrix.Iteration = 0
            IMPES_Matrix.Iteration = 0
            SEQ_Matrix.Iteration = 0
            Initialize(Data)
            Simulation_Cast(Data, History.Simulations(Data.Current_Step - 1), False)
            ...
            Select Case Data.Methodology.Approach
                ...
                Case Simulation_Approach.Fully_Implicit
                    ...
                    Do While (FullyImplicit_MSE.Sw >= Data.Exactness.IterationMSE.Sw Or FullyImplicit_MSE.Sg >= Data.Exactness.IterationMSE.Sg Or FullyImplicit_MSE.Po >= Data.Exactness.IterationMSE.Po) And Not Paused And FI_Matrix.Iteration < Data.Exactness.IterationMSE.Maximum

                        Create_Implicit_Matrix(History.Simulations(History.Current - 1), Data, FI_Matrix)
                        Solve_Orthomin(FI_Matrix.J, FI_Matrix.Delta_X, FI_Matrix.Mines_R, Data.Exactness.OrthominMaxIteration, Data.Exactness.Equation)
                        FullyImplicit_MSE = Calculate_Fully_Implicit_Iteration_MSE(FI_Matrix)
                        FullyImplicit_Update_Simulatin_Data(Data, FI_Matrix)
                        Initialize(Data, False)
                        RaiseEvent Iteration(History.Simulations(History.Current - 1), Data, FI_Matrix, FullyImplicit_MSE, IMPES_Matrix, IMPES_MSE)

                        'strx += Data.Current_Step.ToString + Chr(Keys.Tab)
                        'strx += Data.Model.Grids(0).Saturation.Water.ToString + Chr(Keys.Tab)
                        'strx += Data.Model.Grids(1).Saturation.Water.ToString + Chr(Keys.Tab)
                        'For ii As Integer = 0 To 3
                        '    strx += Data.Model.Grids(ii).SCAL.pcow.ToString + Chr(Keys.Tab)
                        'Next
                        'strx += vbCrLf

                    Loop

                    'If Data.Model.Grids(19).Saturation.Water > 0.84 Then Data.Model.Grids(19).Saturation.Water = 0.84


                    Data.CurrentIterationCount = FI_Matrix.Iteration

                    If FI_Matrix.Iteration = Data.Exactness.IterationMSE.Maximum Then
                        'Stop
                    End If

                    If Data.Current_Step > Data.Options.PPDA_i Then
                        Data.Time_Steps(0) = Data.Options.PPDA_New
                    End If
                ...
                Case Simulation_Approach.IMPES
                    ...
                    Do While (IMPES_MSE.Po >= Data.Exactness.IterationMSE.Po Or IMPES_Matrix.Iteration <= 1) And Not Paused And IMPES_Matrix.Iteration < Data.Exactness.IterationMSE.Maximum
                        ...
                        Initialize(Data, False)
                        RaiseEvent Iteration(History.First, Data, FI_Matrix, FullyImplicit_MSE, IMPES_Matrix, IMPES_MSE)
                ...
                Case Simulation_Approach.SEQ
                    ...
                    '    strx += Data.Model.Grids(ii).Pressure.Oil.ToString + Chr(Keys.Tab)
                    'Next
                    ''strx += Data.Model.Grids(1).Saturation.Water.ToString
                    ''strx += TT.ToString
                    'strx += vbCrLf

                    'Dim TotalMass As Decimal
                    'TotalWaterMassInjected = 0
                    'TotalOilMassInjected = 0
                    'For ii As Integer = 0 To SizeOf(Data.Model.Grids) - 1
                    '    TotalWaterMassInjected += Data.Model.Grids(ii).Saturation.Water * Data.Model.Grids(ii).Porosity * Data.Model.Grids(ii).Volume * Data.Model.Grids(ii).PVT.Water.Density
                    '    TotalOilMassInjected += Data.Model.Grids(ii).Saturation.Oil * Data.Model.Grids(ii).Porosity * Data.Model.Grids(ii).Volume * Data.Model.Grids(ii).PVT.Oil.Density
                    '    'TotalOilMassInjected += Data.Model.Grids(ii).Flow.Oil * Data.Model.Grids(ii).PVT.Oil.FVF * Data.Model.Grids(ii).PVT.Oil.Density

                    'Next
                    ''strx += TotalWaterMassInjected.ToString + Chr(Keys.Tab)
                    'strx += TotalWaterMassInjected.ToString + Chr(Keys.Tab) + TotalOilMassInjected.ToString + vbCrLf
            ...
            '    '    'TotalTime += Data.Time_Step
            '    '    'OilDis += Data.Model.Grids(15).Flow.Oil * Data.Time_Step
            'strx += Data.Model.Grids(i).Pressure.Oil.ToString + Chr(Keys.Tab)
            'strx += Data.Model.Grids(i).Pressure.Oil.ToString + Chr(Keys.Tab)

            'Next
            'strx += FI_Matrix.Iteration.ToString
            'strx += Data.Model.Grids(19).Pressure.Oil.ToString + Chr(Keys.Tab)
            'strx += Data.Current_Step.ToString + Chr(Keys.Tab) + FI_Matrix.Iteration.ToString
            'strx += vbCrLf
            'Clipboard.SetText(strx)
            'TotalTime += Data.Time_Step
            'OilDis += Data.Model.Grids(SizeOf(Data.Model.Grids) - 1).Flow.Oil * Data.Time_Step
            'strx += TotalTime.ToString + Chr(Keys.Tab) + OilDis.ToString     '+ Chr(Keys.Tab)
            'Static jj As Integer
            'jj += 1
            'If jj Mod 20 = 0 Then
            'For ii As Integer = 0 To SizeOf(Data.Model.Grids) - 1
            '        'Dim Sw1_1 As Decimal = History.Simulations(History.Current - 1).Model.Grids(ii).Saturation.Water
            '        'Dim Sw1_2 As Decimal = History.Simulations(History.Current - 1).Model.Grids(ii + 1).Saturation.Water
            '        'Dim Sw2_1 As Decimal = Data.Model.Grids(ii).Saturation.Water
            '        'Dim Sw2_2 As Decimal = Data.Model.Grids(ii + 1).Saturation.Water
            '        'strx += ((Sw2_2 - Sw2_1) - (Sw1_2 - Sw1_1)).ToString + Chr(Keys.Tab)
            'strx += Data.Model.Grids(ii).Saturation.Water.ToString + Chr(Keys.Tab)
            'Next
            'strx += vbCrLf
    ...
    End Function
    Sub Copy_Deflection(ByVal IM As Fully_Implicit_Matrix)
        Dim str As String
    ...
    End Sub
    Sub Copy_Matrix(ByVal IM As Fully_Implicit_Matrix)
        Dim str As String
    ...
    Sub Create_Implicit_Matrix(ByVal Data_n0 As Simulation, ByRef Data_n1 As Simulation, ByRef IM As Fully_Implicit_Matrix)
        ...
        For nGrid As Integer = 0 To Data_n1.Model.Grids.Length - 1
            ...
            For iAdjacent As Integer = 0 To Data_n1.Model.Grids(nGrid).Adjacents.Length - 1
                ...
                'Dim ii As Integer = Data_n0.Model.Grids(nGrid).Adjacents(iAdjacent).Target_Grid_Index
                'ntx += nGrid.ToString + Chr(Keys.Tab) + Data_n0.Model.Grids(ii).SCAL.pcow.ToString + Chr(Keys.Tab) + Data_n1.Model.Grids(ii).SCAL.pcow.ToString + Chr(Keys.Tab) + ii.ToString + Chr(Keys.Tab) + Data_n1.Model.Grids(ii).Saturation.Water.ToString
                'ntx += vbCrLf
    ...
    Sub Calculate_F_Bracet_Parameters(ByVal data_n As Simulation, ByVal Data_v As Simulation, ByVal nGrid As Integer, ByVal iAdjacent As Integer, ByRef Grid_Params As Grid_Parameters)
        ...
        'If m_Grid.Saturation.Water > 0 Then
        '    stx += m_Grid.Saturation.Water.ToString + Chr(Keys.Tab) + Grid_Params.R.Pcow_Prime_m.ToString + Chr(Keys.Tab)
        'End If
        ...
        'If n_Grid.Saturation.Water > 0 Then
        '    stx += n_Grid.Saturation.Water.ToString + Chr(Keys.Tab) + Grid_Params.R.Pcow_Prime_n.ToString + vbCrLf
        'End If

        'If stx <> "" Then Clipboard.SetText(stx)
